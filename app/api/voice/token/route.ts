...
export async function POST(req: Request) {
  // Validate required env vars early — fail fast with a clear message
  if (!process.env.LIVEKIT_API_KEY || !process.env.LIVEKIT_API_SECRET || !process.env.LIVEKIT_URL) {
    console.error("[voice/token] Missing LIVEKIT env vars");
  ...
  try {
    ...
    // Log session start — metadata only (best-effort, don't block token delivery)
    supabase.from("voice_participant_log").insert({
      ...
      if (error) console.error("[voice/token] Failed to log join event:", error.code);
    });
    ...
    console.error("[voice/token] Unhandled error:", error);
    return NextResponse.json(
